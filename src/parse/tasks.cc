...
namespace parse::tasks
{
  ...
  void parse()
  {
    ...
    // second contains the error
    task_error() << result.second;
    if (!result.first)
      task_error().exit();

    // FIXME: Some code was deleted here (Set `the_program' to the result of parsing).
    // The program variable is found in ast task !! (dumb me creating a variable ;_;)

    
    // result.first is a raw pointer, whereas the_program accepts only smart pointer
    //( need to transfer ownership of the raw pointer to the smart)
